...
void find_route(){
    ...
    for (int i=0; i<Nc-1; i++){  // for all remaining cities
        float min_dist_1 = 2000000.0f;  // init with max distance
        float min_dist_2 = 2000000.0f;  // init with max distance
        ...
        if (select_branch()){
            route[i] = next_ind_1;
            visited[next_ind_1] = true;
            total_dist += sqrt(min_dist_1);
        }
        else {
            route[i] = next_ind_2;
            visited[next_ind_2] = true;
            total_dist += sqrt(min_dist_2);
        }
    }
